...
class HistoryMigrator {
  ...
  async checkPrerequisites() {
    ...
    if (!existsSync(this.tursoConfigFile)) {
      ...
      console.log(
        chalk.yellow('Execute primeiro: node libs/turso-client-setup.js'),
      );
      return false;
    }
  ...
  async loadLocalHistory() {
    try {
      ...
      console.error(
        chalk.red(`❌ Erro ao ler histórico local: ${error.message}`),
      );
      return null;
    }
  }
  ...
  async initializeTurso() {
    try {
      ...
      console.error(
        chalk.red(`❌ Erro ao conectar ao Turso: ${error.message}`),
      );
      return null;
    }
  }
  ...
  async migrate() {
    try {
      ...
      try {
        ...
        console.error(
          chalk.yellow(`⚠️  Não foi possível salvar o log: ${error.message}`),
        );
      }
