 ...
 * @param baseKey - Base key for React elements
 * @returns Array of React elements
 */
export const parseMarkdownToElements = (text: string, baseKey: string = 'md'): ReactElement[] => {
 ...
 * @param baseKey - Base key for elements
 * @returns Array of React elements
 */
function processInlineMarkdown(text: string, baseKey: string): ReactElement[] {
  const elements: ReactElement[] = [];
  let remaining = text;
  let keyIndex = 0;
  ...
  segments.forEach((segment: SegmentItem) => {
  ...
  processedSegments.forEach((segment: SegmentItem, idx: number) => {
...
// Type for the exported module
interface MarkdownParserModule {
  parseMarkdownToElements: (text: string, baseKey?: string) => ReactElement[];
  clearMarkdownCache: () => void;
}

const markdownParser: MarkdownParserModule = {
