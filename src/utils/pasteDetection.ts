 ...
 */
export function enableBracketedPasteMode(isTTY: boolean, isDebug: boolean = false): void {
  if (isTTY) {
 ...
 * @param {boolean} isDebug - Whether debug mode is active
 */
export function disableBracketedPasteMode(isTTY: boolean, isDebug: boolean = false): void {
  if (isTTY) {
 ...
 */
export function isPastedContent(char: string): boolean {
 ...
 */
export function processPastedInput(currentInput: string, pastedContent: string): string {
