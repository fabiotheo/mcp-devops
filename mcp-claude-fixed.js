...
/**
 * MCP Terminal Assistant - Claude Code Style (FIXED)
...
/**
 * Main Interface - Simpler approach
 */
class MCPClaudeFixed {
    constructor() {
        this.rl = null;
        this.config = {};
        this.aiModel = null;
        ...
        this.QUEUE_TIMEOUT = 100; // 100ms timeout to detect paste
    ...
    setupReadline() {
        ...
        // Intercept readline to handle paste
        this.interceptPaste();
    ...
    interceptPaste() {
        ...
        this.rl.on('line', (input) => {
            ...
            // Start or continue collecting paste
            if (timeSinceLastLine < this.QUEUE_TIMEOUT || this.queueTimer) {
                isCollectingPaste = true;
                ...
                // Add line to queue
                this.lineQueue.push(input);

                // Set timer to process queue after paste completes
                this.queueTimer = setTimeout(() => {
                    isCollectingPaste = false;
                    this.processQueue();
                }, this.QUEUE_TIMEOUT);
            } else {
        ...
        // Intercept keypress to capture the last line
        process.stdin.on('keypress', (str, key) => {
            if (!key) return;

            // If we're collecting paste and user presses Enter
            if (isCollectingPaste && key.name === 'return') {
                // Check if there's text in the current line buffer
                if (this.rl.line && this.rl.line.trim()) {
                    // This is the last line of the paste
                    // It will be handled by the 'line' event
                }
            }
        });
    }
    ...
    handlePasteConfirmation(input) {
        ...
        if (input === '') {
            ...
            // User typed something else - treat as cancel
            this.cancelPaste();
        }
    }
    ...
    processQueue() {
        ...
        // Only show paste dialog for multiple lines
        if (lines.length > 1) {
            // Multiple lines = paste detected
            // Clear the terminal line to remove any artifacts
    ...
    showPasteDialog(lines) {
        ...
        this.pasteBuffer = lines;
        this.waitingForPasteConfirmation = true;

        // Clear and show paste dialog
        console.log(chalk.yellow('\n📋 Multi-line paste detected:'));
        console.log(chalk.gray('━'.repeat(50)));

        // Show preview of pasted lines
        ...
        if (lines.length > maxPreviewLines) {
            console.log(chalk.gray(`  ... and ${lines.length - maxPreviewLines} more lines`));
        }
        ...
        // Change prompt to indicate waiting for confirmation
...
${chalk.gray('Type your question for AI assistance')}
    ...
    showHeader() {
        ...
        console.log(chalk.green('✨ Auto-detects paste and groups lines'));
        console.log();
    }
