...
class TursoSchemaVerifier {
    ...
    async loadConfig() {
        if (!existsSync(this.configPath)) {
            ...
            console.log(chalk.green('  Para administrador: node libs/turso-admin-setup.js'));
            console.log(chalk.green('  Para cliente: node libs/turso-client-setup.js'));
            return false;
        }
    ...
    async verifyTables() {
        ...
        for (const table of requiredTables) {
            try {
                ...
                this.errors.push(`  ❌ Erro ao verificar tabela '${table}': ${error.message}`);
            }
        }
    }
    ...
    async verifyIndexes() {
        ...
        for (const index of expectedIndexes) {
            try {
                ...
                this.warnings.push(`  ⚠️  Erro ao verificar índice '${index}': ${error.message}`);
            }
        }
    }
    ...
    async verifyPermissions() {
        ...
        try {
            ...
            if (error.message.includes('PERMISSION')) {
                ...
                this.errors.push('  ❌ Schema não existe - execute turso-admin-setup.js primeiro');
            } else {
        ...
        try {
            ...
            this.warnings.push('  ⚠️  Token parece ter permissões de ADMIN (considere usar token de cliente)');
        } catch {
            this.info.push('  ✅ Token não tem permissões de admin (bom para cliente)');
        }
    }
    ...
    async verifyData() {
        ...
        try {
            ...
            if (userCount === 0) {
                ...
                this.info.push('     Use: ipcom-chat user create --username USER --name "Nome" --email email');
            } else {
                this.info.push(`  ✅ ${userCount} usuário(s) cadastrado(s)`);

                // Listar usuários
                const userList = await this.client.execute('SELECT username, name, email FROM users LIMIT 5');
                for (const user of userList.rows) {
                    this.info.push(`     - ${user.username} (${user.name}) - ${user.email}`);
                }
                if (userCount > 5) {
        ...
        try {
            ...
            const machineCount = machines.rows[0].count;
            ...
            if (machineCount === 0) {
                ...
                for (const machine of machineList.rows) {
                    this.info.push(`     - ${machine.hostname} (${machine.ip_address || 'N/A'}) - Visto: ${machine.last_seen}`);
                }
                if (machineCount > 5) {
        ...
        try {
            ...
            const historyCount = history.rows[0].count;
    ...
    async showReport() {
        ...
        if (this.config) {
            ...
            console.log(chalk.gray(`  Modo: ${this.config.history_mode || 'padrão'}`));
            console.log(chalk.gray(`  Admin: ${this.config.is_admin_config ? 'Sim' : 'Não'}`));
        ...
        if (this.errors.length === 0) {
            ...
            if (this.errors.some(e => e.includes('Schema não existe'))) {
                console.log(chalk.yellow('1. Execute o setup de admin: node libs/turso-admin-setup.js'));
            }
    ...
    async run() {
        console.log(chalk.blue('╔════════════════════════════════════════════════════╗'));
        console.log(chalk.blue('║        VERIFICADOR DE SCHEMA TURSO             ║'));
        console.log(chalk.blue('╚════════════════════════════════════════════════════╝'));
