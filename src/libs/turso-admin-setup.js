...
class TursoAdminSetup {
    ...
    async displayBigWarning() {
        ...
        console.log(chalk.red('║                                                                ║'));
        console.log(chalk.red('║     ⚠️  ⚠️  ⚠️   ATENÇÃO ADMINISTRADOR  ⚠️  ⚠️  ⚠️            ║'));
        console.log(chalk.red('║                                                                ║'));
        console.log(chalk.red('║   ESTE SCRIPT CRIA O SCHEMA DO BANCO DE DADOS!               ║'));
        console.log(chalk.red('║                                                                ║'));
        console.log(chalk.red('║   • Execute APENAS UMA VEZ                                    ║'));
        console.log(chalk.red('║   • Execute APENAS no servidor principal                      ║'));
        console.log(chalk.red('║   • NÃO execute em máquinas clientes                          ║'));
        console.log(chalk.red('║   • NÃO execute múltiplas vezes                               ║'));
        console.log(chalk.red('║                                                                ║'));
        console.log(chalk.red('║   Para máquinas clientes use:                                 ║'));
        console.log(chalk.yellow('║   $ node libs/turso-client-setup.js                           ║'));
        console.log(chalk.red('║                                                                ║'));
        console.log(chalk.red('╚════════════════════════════════════════════════════════════════╝'));
        console.log();
    }
    ...
    async confirmAdmin() {
        ...
        const answer1 = await rl.question(chalk.cyan('Você é o ADMINISTRADOR do sistema? (digite "SIM EU SOU ADMIN"): '));
        if (answer1 !== 'SIM EU SOU ADMIN') {
            console.log(chalk.red('\n❌ Resposta incorreta. Abortando...'));
            console.log(chalk.yellow('Use: node libs/turso-client-setup.js para configurar máquinas clientes.'));
            rl.close();
            process.exit(1);
        }

        const answer2 = await rl.question(chalk.cyan('Esta é a PRIMEIRA vez que está criando o banco? (digite "SIM PRIMEIRA VEZ"): '));
        if (answer2 !== 'SIM PRIMEIRA VEZ') {
            console.log(chalk.red('\n❌ Se não é a primeira vez, não execute este script!'));
            console.log(chalk.yellow('O schema já deve existir. Use turso-client-setup.js'));
            rl.close();
            process.exit(1);
        }

        const answer3 = await rl.question(chalk.red('CONFIRMA que quer CRIAR O SCHEMA do banco? (digite "CRIAR SCHEMA AGORA"): '));
    ...
    async checkIfAlreadyDone() {
        if (existsSync(this.adminMarkerPath)) {
            ...
            console.log(chalk.yellow('Se precisa recriar o schema, delete o arquivo:'));
    ...
    async getAdminCredentials() {
        ...
        console.log(chalk.cyan('═════════════════════════════════'));
        console.log(chalk.yellow('Obtenha as credenciais em: https://turso.tech/app/databases\n'));
        ...
        while (!token || token.length < 20) {
            token = await rl.question(chalk.cyan('Token de ADMIN (com permissão total): '));
            if (token.length < 20) {
    ...
    async createSchema() {
        ...
        try {
            ...
            await fs.writeFile(this.adminMarkerPath, JSON.stringify({
    ...
    async createAdminUser() {
        ...
        const create = await rl.question(chalk.yellow('Deseja criar o primeiro usuário admin? (s/n): '));
    ...
    async generateClientToken() {
        ...
        console.log(chalk.white('5. Escolha permissões: READ + WRITE (não ADMIN!)'));
    ...
    async run() {
        try {
            ...
            console.log(chalk.green('\n╔════════════════════════════════════════════════════╗'));
            console.log(chalk.green('║           SETUP DE ADMIN CONCLUÍDO!               ║'));
            console.log(chalk.green('╚════════════════════════════════════════════════════╝'));
