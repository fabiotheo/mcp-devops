...
/**
 * MCP Terminal Assistant - Claude Code Style (FINAL SOLUTION)
 * Detecção de paste simples e confiável
 */
...
class MCPClaudeFinal {
    ...
    setupReadline() {
        ...
        // Override _writeToOutput to detect paste
        const originalWriteToOutput = this.rl._writeToOutput;
        this.rl._writeToOutput = function(stringToWrite) {
            // Check if the string contains multiple lines
            if (!this.waitingForConfirmation && stringToWrite && stringToWrite.includes('\n')) {
                const lines = stringToWrite.split('\n');

                // If we have multiple lines being written at once, it's likely a paste
                if (lines.length > 2) {
                    // Clear the current line
                    process.stdout.write('\r' + ' '.repeat(80) + '\r');

                    // Extract the actual content (remove prompt from first line)
                    const content = lines.map((line, i) => {
                        if (i === 0) {
                            // First line might have the prompt, remove it
                    ...
                    if (content.length > 1) {
                        // Cancel the normal output
                        this.line = '';
                        this.cursor = 0;

                        // Show paste dialog
                        setTimeout(() => {
                            this.showPasteDialog(content);
                        }, 10);
    ...
    showPasteDialog(lines) {
        ...
        // If single line, just process it
        ...
        this.pendingPaste = nonEmptyLines;
    ...
    handlePasteConfirmation(input) {
        if (input === '') {
            ...
            console.log(chalk.green('✓ Sending...'));
            console.log();
