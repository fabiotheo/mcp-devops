 ...
 */
function getFileMapping(config) {
    const baseFiles = [
 ...
 */
export async function installFiles(config, options = {}) {
    ...
    for (const mapping of mappings) {
        ...
        try {
            ...
            console.error(`❌ Erro ao instalar ${mapping.dest}: ${error.message}`);
            throw error;
        }
    }
}
 ...
 */
export async function setPermissions(config, files) {
    for (const file of files) {
        ...
        if (await fileExists(filePath)) {
            try {
                ...
                console.error(`⚠️ Erro ao definir permissões para ${file}: ${error.message}`);
            }
        }
    }
}
 ...
 */
export async function verifyInstallation(config) {
