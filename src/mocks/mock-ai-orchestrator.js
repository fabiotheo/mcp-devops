      ...
      patterns: [
        {
          ...
          response: {
            commands: [
              ...
              {
                command: 'sudo fail2ban-client status apache-auth',
                output:
                  'Currently banned: 2\nBanned IP list: 203.0.113.45 198.51.100.78',
              },
              {
                command: 'sudo fail2ban-client status nginx-http-auth',
                output: 'Currently banned: 1\nBanned IP list: 172.217.0.14',
              },
            ],
            finalAnswer:
              'There are 6 IPs currently blocked across all fail2ban jails:\n- sshd: 3 IPs\n- apache-auth: 2 IPs\n- nginx-http-auth: 1 IP',
          },
        },
        {
          ...
          response: {
            ...
            finalAnswer:
              'You have 2 running containers (web-server, cache-server) and 1 stopped container (db-backup).',
          },
        },
        {
  ...
  async processCommand(command, context = {}) {
    return new Promise(resolve => {
      ...
      setTimeout(async () => {
        ...
        const match = this.responses.patterns.find(p =>
    ...
    const patterns = [
      {
        type: 'fail2ban',
        confidence: 0.9,
        matched: /fail2ban|blocked/i.test(input),
      },
      {
        type: 'docker',
        confidence: 0.85,
        matched: /docker|container/i.test(input),
      },
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  orchestrator.on('pattern-detected', pattern => {
    console.log(
      `[PATTERN] Detected: ${pattern.type} (confidence: ${pattern.confidence})`,
    );
  });
