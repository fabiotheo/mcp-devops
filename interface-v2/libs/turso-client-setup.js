...
class TursoClientSetup {
    ...
    async displayClientBanner() {
        console.clear();
        console.log(chalk.blue('╔════════════════════════════════════════════════════════════════╗'));
        console.log(chalk.blue('║                                                                ║'));
        console.log(chalk.blue('║            CONFIGURAÇÃO DE MÁQUINA CLIENTE                    ║'));
        console.log(chalk.blue('║                                                                ║'));
        console.log(chalk.blue('║   Este script configura a conexão com Turso                   ║'));
        console.log(chalk.blue('║   NÃO cria schema - apenas conecta ao banco existente         ║'));
        console.log(chalk.blue('║                                                                ║'));
        console.log(chalk.blue('╚════════════════════════════════════════════════════════════════╝'));
        console.log();
    }
    ...
    async checkExistingConfig() {
        if (existsSync(this.configPath)) {
            ...
            if (config.is_admin_config) {
                ...
                console.log(chalk.yellow('Não é necessário executar o setup de cliente.'));
            ...
            const answer = await rl.question(chalk.cyan('Deseja sobrescrever a configuração existente? (s/n): '));
            rl.close();
    ...
    async getClientCredentials() {
        ...
        console.log(chalk.gray('Token de cliente tem apenas permissões READ + WRITE\n'));

        let url = '';
        while (!url || !url.includes('libsql://')) {
            url = await rl.question(chalk.cyan('URL do Banco Turso (deve começar com libsql://): '));
        ...
        while (!token || token.length < 20) {
            ...
            if (token.length < 20) {
                console.log(chalk.red('❌ Token muito curto! Verifique se copiou corretamente.'));
            }
        }
    ...
    async testConnection(credentials) {
        ...
        try {
            ...
            if (error.message.includes('PERMISSION_DENIED')) {
                ...
                console.log(chalk.yellow('Certifique-se de usar um token de CLIENTE com READ + WRITE.'));
            } else {
    ...
    async generateMachineId() {
        ...
        const machineId = crypto.createHash('sha256').update(uniqueString).digest('hex');
        ...
        await fs.writeFile(this.machineIdPath, JSON.stringify(machineData, null, 2));
        console.log(chalk.green(`\n🔑 Machine ID gerado: ${machineId}`));
        
        return machineId;
    }

    async registerMachine(machineId) {
        ...
        try {
            ...
            if (existing.rows.length > 0) {
                ...
                console.log(chalk.green('✅ Máquina já registrada - informações atualizadas'));
            } else {
            ...
            console.log(chalk.gray('A máquina será registrada automaticamente no primeiro uso.'));
        }
    }
    ...
    async saveConfig(credentials, machineId) {
    ...
    async askForUsername() {
        ...
        console.log(chalk.cyan('════════════════════════════════'));
        console.log(chalk.yellow('Para usar o sistema, você precisa de um usuário.\n'));

        const hasUser = await rl.question(chalk.cyan('Você já tem um usuário cadastrado? (s/n): '));
        ...
        if (hasUser.toLowerCase() === 's') {
            ...
            console.log(chalk.green('  ipcom-chat user create --username SEU_USER --name "Seu Nome" --email seu@email.com'));
        }
    ...
    async showNextSteps() {
        console.log(chalk.green('\n╔════════════════════════════════════════════════════╗'));
        console.log(chalk.green('║         CONFIGURAÇÃO DE CLIENTE CONCLUÍDA!        ║'));
        console.log(chalk.green('╚════════════════════════════════════════════════════╝'));

        console.log(chalk.yellow('\n📋 PRÓXIMOS PASSOS:'));
        console.log(chalk.white('\n1. Se você não tem usuário:'));
        console.log(chalk.gray('   ipcom-chat user create --username USER --name "Nome" --email email@example.com'));
        ...
        console.log(chalk.gray('   ipcom-chat history search TERMO  # Buscar no histórico'));
        console.log(chalk.gray('   ipcom-chat machine info       # Informações da máquina'));
        console.log(chalk.gray('   ipcom-chat user list          # Listar usuários'));
        ...
        try {
            ...
            if (isAuto) {
                ...
                if (!existsSync(this.configPath)) {
                    ...
                    console.log('   Execute manualmente: node libs/turso-client-setup.js');
                    return;
                }
                const config = JSON.parse(await fs.readFile(this.configPath, 'utf8'));
            ...
            console.error(chalk.yellow('2. Token tem permissões de CLIENTE (READ + WRITE)'));
