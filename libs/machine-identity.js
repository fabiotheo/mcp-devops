...
export default class MachineIdentityManager {
    constructor(options = {}) {
        this.cacheDir = options.cacheDir || path.join(os.homedir(), '.mcp-terminal');
        ...
        if (existsSync(this.cacheFile)) {
            try {
                ...
                if (cachedId && cachedId.trim()) {
                    if (this.debug) console.log('Machine ID loaded from cache:', cachedId.trim());
    ...
    async getSystemMachineId() {
        ...
        for (const filepath of possiblePaths) {
            try {
                ...
                if (this.debug) console.log(`Could not read ${filepath}:`, error.message);
            }
        }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    (async () => {
        ...
        switch (command) {
            ...
            default:
                console.log('Usage: node machine-identity.js [generate|info|invalidate]');
