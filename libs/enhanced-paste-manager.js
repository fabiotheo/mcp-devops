...
class EnhancedPasteManager {
    ...
    setupInputHandling() {
        ...
        console.log(chalk.yellow('📋 Bracketed paste temporarily disabled - use /paste command'));
    }
    ...
    startManualPaste() {
        if (this.isInManualPaste) {
            this.readline.write(chalk.yellow('📝 Já está em modo paste. Use linha vazia dupla para finalizar.\n'));
        ...
        this.readline.write(chalk.gray('   → Pressione Enter duas vezes seguidas para finalizar\n'));
    ...
    endManualPaste() {
        ...
        if (this.buffer.length > 0 && this.buffer[this.buffer.length - 1].trim() === '') {
            this.buffer.pop();
        }
        ...
        if (content) {
            ...
            this.readline.write(chalk.gray('📝 Modo paste cancelado (sem conteúdo)\n'));
            this.readline.prompt();
        }
    }
    ...
    processPastedContent(content) {
        ...
        this.readline.write('\n');
        this.readline.write(chalk.green(`✅ Conteúdo salvo como anexo ${placeholder}\n`));
        this.readline.write(chalk.gray(`   ${lines.length} linhas, ${content.length} caracteres\n`));
        this.readline.write(chalk.gray(`   Use /expand ${placeholder} para ver o conteúdo\n`));
    ...
    showBufferPreview() {
        ...
        const preview = this.buffer.slice(-2).map(line =>
            line.length > 60 ? line.substring(0, 57) + '...' : line
        );

        // Limpar linha atual e mostrar preview
        this.readline.write(`\r${chalk.gray(`[${lineCount} linhas] ${preview.join(' | ')}`)} `);
    }
