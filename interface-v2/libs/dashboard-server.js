...
class DashboardServer {
    ...
    async initialize() {
        try {
            ...
            try {
                ...
                console.log(chalk.yellow('⚠️  Turso not available, using local cache only'));
            }
            ...
            console.error(chalk.red('❌ Failed to initialize dashboard server:'), error);
            return false;
        }
    }
    ...
    setupWebSocket() {
        this.io.on('connection', (socket) => {
            ...
            console.log(chalk.gray(`[WS] Client connected (${this.connectedClients} total)`));
            ...
            socket.on('disconnect', () => {
                this.connectedClients--;
                console.log(chalk.gray(`[WS] Client disconnected (${this.connectedClients} total)`));
            });
        });
    }
    ...
    async searchHistory(query) {
        ...
        const results = allHistory.filter(item =>
            ...
            (item.response && item.response.toLowerCase().includes(query.toLowerCase()))
        );
