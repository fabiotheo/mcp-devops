...
class AIConfigurator {
    ...
    async loadExistingConfig() {
        if (existsSync(CONFIG_PATH)) {
            try {
                ...
                console.log('⚠️  Erro ao ler configuração existente, iniciando nova configuração...');
            }
        }
        return false;
    }
    ...
    async selectProvider() {
        ...
        const choice = await this.question('Digite o número da sua escolha (1-5): ');
    ...
    async getApiKey(provider) {
        ...
        if (this.config[existingKeyField] && this.config[existingKeyField] !== `YOUR_${provider.toUpperCase()}_API_KEY`) {
            const keepExisting = await this.question('Já existe uma API key configurada. Deseja mantê-la? (S/n): ');
            if (keepExisting.toLowerCase() !== 'n') {
                return this.config[existingKeyField];
            }
        }

        const apiKey = await this.question('Cole sua API key aqui (será ocultada): ');
    ...
    async configureWebSearch() {
        ...
        const currentStatus = this.config.web_search?.enabled ? 'ativada' : 'desativada';
        console.log(`Status atual: ${currentStatus}\n`);

        const enable = await this.question('Deseja ativar a busca na web para melhorar as respostas? (S/n): ');
    ...
    async run() {
        ...
        console.log(`   • Web Search: ${webSearchEnabled ? 'Ativado' : 'Desativado'}`);
        console.log('\n🚀 Comandos úteis:');
