...
class SyncManager {
        ...
        try {
            ...
            if (this.config.debug) {
                ...
                if (this.lastSyncTime) {
                    console.log(chalk.gray(`Last sync: ${new Date(this.lastSyncTime).toLocaleString()}`));
                }
            }
            ...
            console.error(chalk.red('❌ SyncManager initialization failed:', error.message));
            return false;
        }
    }
    ...
    startPeriodicSync() {
        ...
        if (this.config.debug) {
            console.log(chalk.gray(`[Sync] Periodic sync started (${this.config.syncInterval}ms interval)`));
        }
    }
    ...
    async sync() {
        ...
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.green(`[Sync:${syncId}] Complete:`,
    ...
    async uploadToTurso() {
        ...
        try {
            ...
            const pendingItems = this.localCache.getPendingSync(this.config.batchSize);
            ...
            if (this.config.debug) {
                console.log(chalk.gray(`[Sync] Uploading ${pendingItems.length} items...`));
            }
            ...
            for (const item of pendingItems) {
                try {
                    ...
                    try {
                        ...
                        console.error(chalk.red(`[Sync] Invalid JSON in sync queue item ${item.id}:`, parseError.message));
                    ...
                    if (this.config.debug) {
                        console.log(chalk.yellow(`[Sync] Upload failed for ${item.id}:`, error.message));
                    }
                }
            }
    ...
    async downloadFromTurso() {
        ...
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.gray(`[Sync] Downloading since ${new Date(since * 1000).toLocaleString()}...`));
            }
            ...
            if (this.config.debug) {
                console.log(chalk.gray(`[Sync] Processing ${remoteHistory.length} remote items...`));
            }
            ...
            for (const remoteItem of remoteHistory) {
                ...
                if (conflict) {
                    // Resolve conflict
                    const resolved = await this.resolveConflict(conflict.local, remoteItem);
     ...
     */
    async checkConflict(remoteItem) {
        try {
            const localItem = this.localCache.db.prepare(`
                SELECT * FROM history_cache
                WHERE command_uuid = ?
            `).get(remoteItem.command_uuid || remoteItem.id);
            ...
            if (localItem.command !== remoteItem.command ||
    ...
    async resolveConflict(localItem, remoteItem) {
        if (this.config.conflictStrategy === 'last-write-wins') {
            ...
            const winner = localItem.timestamp > remoteItem.timestamp ? localItem : remoteItem;

            // Log conflict resolution
            this.localCache.db.prepare(`
            ...
            if (this.config.debug) {
                console.log(chalk.yellow(`[Sync] Conflict resolved:`,
    ...
    async saveCommand(command, response, metadata = {}) {
        ...
        if (!this.isSyncing) {
            setImmediate(() => {
                this.sync().catch(err => {
                    if (this.config.debug) {
                        console.log(chalk.yellow('[Sync] Background sync failed:', err.message));
                    }
                });
            });
        }
    ...
    async getHistory(options = {}) {
        try {
            ...
            if (this.tursoClient && this.tursoClient.client) {
                const tursoHistory = await this.tursoClient.getHistory(options.limit || 100);
