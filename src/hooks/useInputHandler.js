  ...
  useInput(async (char, key) => {
    ...
    if (isDebug) {
      if (char) {
        ...
        debug('Char codes', Array.from(char).map(c => c.charCodeAt(0)));
        ...
        if (action) {
          ...
          switch (action.type) {
            case 'SHOW_HELP':
              ...
              setHistory(prev => [...prev, `❯ ${command}`, formatResponse(action.payload.text, debug)]);
              break;
            ...
            case 'SHOW_HISTORY':
              ...
              setHistory(prev => [...prev, `❯ ${command}`, formatResponse(historyText, debug)]);
              break;
...
Config: ${action.payload.config}`;
              ...
              setHistory(prev => [...prev, `❯ ${command}`, formatResponse(statusText, debug)]);
              break;
            ...
            case 'UNKNOWN_COMMAND':
              ...
              setHistory(prev => [...prev, `❯ ${command}`, formatResponse(errorText, debug)]);
              break;
          }
        }
      } else if (command) {
