...
class LocalCache {
    ...
    async initialize() {
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.green('✅ LocalCache initialized at:', this.config.dbPath));
            }
        } catch (error) {
            console.error(chalk.red('❌ LocalCache initialization failed:', error.message));
            throw error;
        }
    }
    ...
    async saveCommand(command, response, metadata = {}) {
        ...
        if (command.length > MAX_COMMAND_SIZE) {
            console.error(chalk.yellow(`[Cache] Command too large (${command.length} chars), truncating`));
            command = command.substring(0, MAX_COMMAND_SIZE) + '...[truncated]';
        }

        if (response && response.length > MAX_RESPONSE_SIZE) {
            console.error(chalk.yellow(`[Cache] Response too large (${response.length} chars), truncating`));
        ...
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.gray(`[Cache] Saved: ${command.substring(0, 50)}...`));
            }
    ...
    getPendingSync(limit = 100) {
        try {
            ...
            console.error(chalk.red('[Cache] Get pending sync error:', error.message));
            return [];
        }
    }
    ...
    getMetadata(key) {
        try {
            ...
            try {
                ...
                console.error(chalk.red('[Cache] Failed to parse metadata:', parseError.message));
                return null;
            }
        } catch (error) {
            return null;
    ...
    importHistory(remoteHistory) {
        ...
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.green(`[Cache] Imported ${remoteHistory.length} items`));
            }
            return true;
        } catch (error) {
    ...
    cleanup(daysToKeep = 30) {
        ...
        try {
            ...
            if (this.config.debug) {
                console.log(chalk.gray(`[Cache] Cleaned ${result.changes} old entries`));
            }
    ...
    close() {
        if (this.db) {
            ...
            try {
                ...
                console.error(chalk.red('[Cache] Error finalizing statements:', error.message));
            }
