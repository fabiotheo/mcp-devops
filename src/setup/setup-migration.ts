...
export interface MigrationStep {
  version: string;
  description: string;
  apply: (config: MigrationConfig) => Promise<void>;
  rollback?: (config: MigrationConfig) => Promise<void>;
}
...
export class MigrationManager {
   ...
   */
  private async detectLegacyVersion(): Promise<string | null> {
   ...
   */
  private async extractVersionFromSetup(setupPath: string): Promise<string | null> {
    try {
      const content = await fs.readFile(setupPath, 'utf8');

      // Pattern 1: version: "1.2.3"
      const versionMatch1 = content.match(/version:\s*["']([^"']+)["']/);
      if (versionMatch1) return versionMatch1[1];
      ...
      if (versionMatch3) return versionMatch3[1];
   ...
   */
  async createBackup(customTag?: string): Promise<BackupInfo> {
   ...
   */
  private async migrateToSimplified(config: MigrationConfig): Promise<void> {
   ...
   */
  private async rollbackFromSimplified(config: MigrationConfig): Promise<void> {
    if (!config.backupDir) {
